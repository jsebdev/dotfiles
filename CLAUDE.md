...
- **mind_palace_practice** - Interactive drill for rehearsing memory material, living in `shared/my_tools/mind_palace_ritual/`. It is a `uv` project, run through `dotfiles_scripts/mind_palace_practice`, a wrapper that `setup_mind_palace_practice.sh` symlinks into `~/.local/bin`. It practices two disciplines, mind palaces and digit sprints, each a class implementing the `Discipline` protocol in `mind_palace_practice/disciplines.py` (`name`, `can_practice()`, `practice()`, `has_best_times()`, `show_best_times()`); `cli.py` asks "What do you want to practice?" (and, for `--best-times`, "Which best times do you want to see?"), skipping the question when only one discipline has something to practice. Mind palaces (`palace_discipline.py`) are the original technique. Adding one means adding one module under `resources/mind_palaces/` that exposes a single `mind_palace` variable, since `catalog.py`'s generic `discovered(package, attribute)` (shared with number sequences) finds them at runtime. Objects are indexed from 0 in every palace, and the drill calls that position an index rather than a number, in both ritual names (`Index to object`, `Object to index`) and every message. `Cloud Needle Isle` relies on that zero based indexing: its rooms hold one decade each, 00 to 09, 10 to 19 and so on, so an object's index is the major system pair it stands for. The "How much do you want to practice?" menu offers the whole palace, the room ranges that `mind_palace_decks.py` enumerates, and `Range of objects`, which asks for a first and a last object index through `index_question.py`: a non numeric answer is rejected as not an index, and anything outside the deck's first and last index (or, for the last one, below the first) as out of range, re-asking until the answer is valid. Those chosen ranges record best times like any other range, but they cannot be enumerated into `board_selection.py`'s catalog, so a board whose key is not in the catalog falls back to matching the palace whose name prefixes the recorded deck name. Best times live in `practice_records/`, which **is committed** so the boards follow the repo across machines. That is a directory holding one file per run, rather than a single file, so two machines recording runs before either pulls create separate files instead of conflicting edits to one. `mind_palace_practice --best-times` browses those boards instead of practicing. It asks the same narrowing questions as the practice menu (mind palace, range, ritual), each with an `All` option, but `board_selection.py` builds every question from the boards that actually have records, so options you have never played are hidden and no combination leads to an empty board. A board is identified by `BoardKey` (deck name, card count, ritual), so renaming a room or a ritual, or adding an object, orphans that board's old records; they stay visible under their recorded deck name rather than disappearing. Digit sprints (`mind_palace_practice/sprint/`) type the decimals of a number sequence one keystroke per digit, no Enter: `keyboard.py`'s `TerminalKeys` puts stdin into cbreak mode so every digit is judged the instant it is typed, Enter and any other non-digit key are ignored, the first wrong digit ends the sprint and is recorded, and Ctrl-C aborts without recording (cbreak mode still lets it raise `KeyboardInterrupt`). Adding a sequence means adding one module under `resources/number_sequences/` that exposes a single `number_sequence` variable (`name`, `whole_part`, `decimals`); `euler.py` holds the verified 10,000 decimals of Euler's number, generated from a source file by a script rather than typed by hand. The clock starts on the first keystroke, and `sprint/milestones.py`'s `MilestoneLadder` (10, 20, 50, 100, then every +100 forever) decides which digit counts get a split time. A sprint is ranked on a Distance board (most correct digits wins, ties broken by lower time then earlier date) plus one board per milestone reached (lower split time wins, then earlier date); `sprint/records.py`'s `kept_runs` keeps a run as long as it sits in the top 10 of any board it belongs to, so a run can drop off the Distance board yet stay recorded for a milestone it holds. Records are JSON files in `practice_records/sprints/`, kept separate from the mind palace records because `RecordFiles`'s glob is not recursive. `tests/` (a `pytest` dev dependency group, run with `uv run --group dev pytest`) covers the milestone ladder, the `Sprint` engine, sprint ranking and `kept_runs`, the sprint record files and `SprintRecordBook.place` round-tripping through a temp directory, and the Euler resource and number-sequence discovery; `keyboard.py` and the terminal UI modules are the untested I/O edges.
