...
alias lrctt="aws ecs list-tasks --cluster rally-claim-cluster-test"
alias lrcst="aws ecs list-tasks --cluster rally-claim-cluster-stage"
...
rallyclaim_run_django_task_terminal() {
    ...
    # local task_id=$(aws ecs list-tasks --cluster $cluster_name --family django-app --query 'taskArns[0]' --output text)
    local task_id=$(aws ecs list-tasks --cluster $cluster_name --query 'taskArns[0]' --output text)
